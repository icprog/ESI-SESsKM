...
int TCPDriver::receiveResponse(SOCKET * connectSocket, SOCKET * listenSocket, char *request, char * response, bool closeConnection, unsigned long int nonBlockingMode, Buffer * b, )
{
	int iResult = -1;
	do {
		...
		if (iResult > 0)
		{
			...
			if (listenSocket == nullptr && b != nullptr) {
			...
			else if(listenSocket == nullptr && b == nullptr){ // server dobija nesto od klijenta
				
			}
			else { // klijent dobija odgovor od servera ako su i buffer i listen socket null
